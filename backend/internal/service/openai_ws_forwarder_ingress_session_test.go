...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_InvalidEncryptedContentRecovery(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	firstConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"error","error":{"type":"invalid_request_error","code":"invalid_encrypted_content","message":"The encrypted content Hand off to README.md could not be verified."}}`),
		},
	}
	secondConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_turn_invalid_encrypted_recover","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
		},
	}
	dialer := &openAIWSQueueDialer{
		conns: []openAIWSClientConn{firstConn, secondConn},
	...
	account := &Account{
		ID:          121,
		Name:        "openai-ingress-invalid-encrypted-content",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	writeCtx, cancelWrite := context.WithTimeout(context.Background(), 3*time.Second)
	err = clientConn.Write(writeCtx, coderws.MessageText, []byte(`{"type":"response.create","model":"gpt-5.1","stream":false,"previous_response_id":"resp_bad_encrypted","input":[{"type":"reasoning","encrypted_content":"Hand off to README.md"},{"type":"input_text","text":"hello"}]}`))
	cancelWrite()
	require.NoError(t, err)
	...
	require.Equal(t, "resp_turn_invalid_encrypted_recover", gjson.GetBytes(message, "response.id").String())
	...
	require.Equal(t, 2, dialer.DialCount(), "invalid_encrypted_content 恢复应换连重试一次")

	firstConn.mu.Lock()
	firstWrites := append([]map[string]any(nil), firstConn.writes...)
	firstConn.mu.Unlock()
	require.Len(t, firstWrites, 1)
	require.True(t, gjson.Get(requestToJSONString(firstWrites[0]), "previous_response_id").Exists())
	require.True(t, gjson.Get(requestToJSONString(firstWrites[0]), "input.0.encrypted_content").Exists())
	...
	require.Len(t, secondWrites, 1)
	secondWrite := requestToJSONString(secondWrites[0])
	require.False(t, gjson.Get(secondWrite, "previous_response_id").Exists(), "非工具续链恢复应移除 previous_response_id")
	require.False(t, gjson.Get(secondWrite, "input.0.encrypted_content").Exists(), "恢复重试不应继续发送失效 encrypted_content")
