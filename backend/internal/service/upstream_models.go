...
func (s *AccountTestService) fetchAntigravityOAuthUpstreamModels(ctx context.Context, account *Account) ([]string, error) {
	...
	if projectID == "" {
		return nil, newUpstreamModelSyncConfigError("No Antigravity project id is available", nil)
	}
	...
	if err != nil {
		return nil, newUpstreamModelSyncConfigError("Failed to encode Antigravity model sync payload", err)
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, strings.TrimRight(baseURL, "/")+"/v1internal:fetchAvailableModels", strings.NewReader(string(reqBody)))
	if err != nil {
		return nil, newUpstreamModelSyncConfigError("Invalid Antigravity model list URL", err)
	}
	req.Header.Set("Accept", "application/json")
	...
	body, err := io.ReadAll(io.LimitReader(resp.Body, upstreamModelsBodyLimit+1))
	if err != nil {
		return nil, newUpstreamModelSyncUpstreamError("Failed to read Antigravity model list", err)
	}
	if int64(len(body)) > upstreamModelsBodyLimit {
		return nil, newUpstreamModelSyncUpstreamError("Antigravity model list response is too large", fmt.Errorf("response exceeds %d bytes", upstreamModelsBodyLimit))
	}
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		return nil, newUpstreamModelSyncUpstreamError(
			fmt.Sprintf("Antigravity model list request failed with HTTP %d", resp.StatusCode),
			fmt.Errorf("antigravity model list returned HTTP %d", resp.StatusCode),
		)
	}
	...
	if err := json.Unmarshal(body, &modelsResp); err != nil {
		return nil, newUpstreamModelSyncUpstreamError("Antigravity model list response was not valid JSON", err)
	}
