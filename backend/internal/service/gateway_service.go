...
type ForwardResult struct {
	...
	// Sora 媒体输出字段（Sora 模型使用）
	MediaType string   // 媒体类型，如 video/image/prompt
	MediaURL  string   // 主媒体 URL
	MediaURLs []string // 多媒体 URL 列表
}
	...
	for {
		...
		case ev, ok := <-events:
			if !ok {
				...
				if !sawTerminalEvent {
					return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, clientDisconnect: clientDisconnected}, streamUsageIncompleteTerminalError(clientDisconnected, streamInterval, &lastReadAt)
				}
...
// 清理 Anthropic API 专有字段、修复 thinking/tool_use ID，并过滤 Bedrock 不支持的 beta token。
func (s *GatewayService) ApplyBedrockCCCompat(c *gin.Context, body []byte, model string, account *Account, groupID *int64) []byte {
	...
	if betaHeader := strings.TrimSpace(c.GetHeader("anthropic-beta")); betaHeader != "" {
...
func streamUsageIncompleteTerminalError(clientDisconnected bool, streamInterval time.Duration, lastReadAt *int64) error {
	if clientDisconnected && streamInterval > 0 && lastReadAt != nil {
		lastRead := time.Unix(0, atomic.LoadInt64(lastReadAt))
		if time.Since(lastRead) >= streamInterval {
			return errors.New("stream usage incomplete after timeout")
		}
	}
