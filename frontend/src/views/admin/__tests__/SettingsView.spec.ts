...
describe("admin SettingsView wechat connect controls", () => {
  ...
  it("hides removed oauth provider controls from the security tab", async () => {
    const wrapper = mountView();
    ...
    expect(wrapper.text()).not.toContain("OIDC 登录");
  });

  it("does not submit removed oauth provider settings on save", async () => {
    const wrapper = mountView();
