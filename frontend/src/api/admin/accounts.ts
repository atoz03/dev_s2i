...
export async function syncUpstreamModels(id: number): Promise<SyncUpstreamModelsResponse> {
  const { data } = await apiClient.post<SyncUpstreamModelsResponse>(`/admin/accounts/${id}/models/sync-upstream`)
  return data
}
